...
parser.add_argument('-batch_size', '--batch_size', default=6, type=int,
                    help='Batch size')
...
def main():
    ...
    print(model)
   # model.load_state_dict(torch.load('mode.pth'), strict=True)
    model.train()
    optimizer = getattr(torch.optim, args.opt)(model.parameters(), **args.opt_params)
    criterion = getattr(criterions, "FocalLoss")
    msg = ''
    if args.resume:
    ...
    for i, data in enumerate(train_loader, args.start_iter):
        ...
        i=1
        for j in range(16):
            o=output[0:1,:,:,:,j].squeeze()
            tt = target[ 0:1, :, :, j].squeeze()
            m=torch.argmax(o,0)*200
            t=m
            t=t.cpu().numpy()
            tt=(tt*200).cpu().numpy()
            gray = Image.fromarray(t.astype('uint8'))
            tt = Image.fromarray(tt.astype('uint8'))
            gray.save("D:\CityU\FYP NN\doc\BraTS-DMFNet-master\\temp\\"+str(i)+"_"+str(j)+".png")
            tt.save("D:\CityU\FYP NN\doc\BraTS-DMFNet-master\\temp\\"+"tt"+str(i)+"_" + str(j) + ".png")
