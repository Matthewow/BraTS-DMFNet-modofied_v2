...
train_set = {
    'root': 'D:\someprogram\BraTS-DMFNet-master\Train',
    'flist': 'all.txt',
}
...
def process_f32(path):
    ...
    label = np.array(nib_load(path + '_segmentation.nii.gz'), dtype='uint8', order='C')
    print(label)
    images = np.stack([
...
def process_f32_2(path):
    ...
    images = np.stack([
        np.array(nib_load(path + '.nii.gz'), dtype='float32', order='C')
        ],-1)
...
def doit2(path):
    for i in range(50):
        path="D:\someprogram\BraTS-DMFNet-master\dataset0\Case"
        p = path + str(i)
        process_f32_2(p)
...
# doit(train_set)
# doit(valid_set)
testSet = 'data/TrainingData2nii/Case'
doit2(testSet)
